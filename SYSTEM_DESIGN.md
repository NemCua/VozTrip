...
### SD-01 Khởi động app & tạo session
...
App                API               DB
 ...
 |<--200 [languages]-----------------|
 |                  |                 |
 | (user chọn ngôn ngữ, đồng ý policy)
 |                  |                 |
 | (app tự sinh UUID làm sessionId)   |
 |                  |                 |
 |--POST /api/sessions--------------->|
 |  { sessionId, languageId }         |
 |                  |--SELECT-------->|  (tìm session đã tồn tại?)
 |                  |<--null----------|
 |                  |--INSERT-------->|  GuestSession
 |                  |<--ok------------|
 |<--200 { sessionId, startedAt }-----|
```

---

### SD-02 Browse danh sách POI & GPS Trigger

```
App                API               DB
 |                  |                 |
 |--GET /api/pois?languageId=..------>|
 |                  |--SELECT-------->|  Poi WHERE IsActive=true
 |                  |                 |  + Media (thumbnail)
 |                  |                 |  + Localizations (localizedName)
 |                  |<--pois list-----|
 |<--200 [pois]----------------------|
 |                  |                 |
 | (GPS: user di chuyển đến gần POI)  |
 |  [khoảng cách < TriggerRadius]     |
 |                  |                 |
 |--POST /api/visitlogs-------------->|
 |  { sessionId, poiId }              |
 |                  |--INSERT-------->|  VisitLog
 ...
 |<--200 { logId }-------------------|
 |                  |                 |
 | (app tự động mở chi tiết POI)      |
```

---

### SD-03 Xem chi tiết POI (Audio / TTS)

```
App                API               DB
 |                  |                 |
 |--GET /api/pois/{id}?languageId=-->|
 |                  |--SELECT-------->|  Poi + Zone + Media + Localizations
 |                  |<--poi detail----|
 |<--200 { poi, media, localizations }|
 |                  |                 |
 | (nếu audioUrl != null)             |
 |--GET audioUrl (Cloudinary)-------->|  (trực tiếp, không qua API)
 |<--audio stream--------------------|
 | (phát audio thuyết minh)           |
 |                  |                 |
 | (nếu audioUrl == null)             |
 | (TTS: app đọc description bằng     |
 |  Text-to-Speech của thiết bị)      |
```

---

### SD-04 Seller đăng ký & Admin duyệt
...
Seller             API               DB         Admin
 ...
 |                  |--AnyAsync------>|  check username duplicate
 |                  |<--false---------|
 |                  |--INSERT-------->|  User (role=seller, IsActive=true)
 |                  |--INSERT-------->|  Seller (ApprovedAt=null)
 |                  |<--ok------------|
 |<--200 "Đăng ký thành công, chờ duyệt"          |
 |                  |                 |            |
 |  (Seller cố login → 403 "chưa duyệt")           |
 |                  |                 |            |
 |                  |                 |            |
 |                  |                 |  |--GET /api/admin/sellers--------->|
 |                  |                 |  |--SELECT sellers ApprovedAt=null--|
 |                  |                 |  |<--pending sellers----------------|
 |                  |                 |  |<--200 [sellers]------------------|
 |                  |                 |            |
 |                  |                 |  |--PUT /api/admin/sellers/{id}/approve->|
 |                  |                 |  |--UPDATE Seller.ApprovedAt=now----|
 |                  |                 |  |<--ok-----------------------------|
 |                  |                 |  |<--200 "Duyệt thành công"---------|
 |                  |                 |            |
 | (Seller đăng nhập lại → thành công)             |
```

---

### SD-05 Seller đăng nhập
...
Seller             API               DB
 ...
 |                  | → true          |
 |                  |                 |
 |                  | check Seller.ApprovedAt != null
 |                  | → đã duyệt      |
 |                  |                 |
 |                  | GenerateToken() → JWT (7 ngày)
 |<--200 { token, userId, role, shopName }
```

---

### SD-06 Seller tạo POI (có kiểm tra plan limit)
...
Seller             API               DB
 ...
 |                  | → true          |
 |                  |                 |
 |                  |--COUNT Pois---->|  WHERE SellerId = sellerId
 |                  |<--count: 1------|
 |                  |                 |
 |                  | count >= FreePlanMaxPois (e.g. 1)?
 |     (nếu vượt)   | → true          |
 |<--400 { code: "PLAN_LIMIT" }-------|
 |                  |                 |
 |     (nếu còn chỗ)| → false         |
...
### SD-07 Upload audio thuyết minh

```
Seller             API               DB         Cloudinary
 |                  |                 |              |
 |--POST /api/seller/pois/{id}/       |              |
 |    localizations/{langId}/audio--->|              |
 |  multipart: file (audio/mp3)       |              |
 |                  |                 |              |
 |                  | verify POI belongs to seller   |
 |                  |--SELECT-------->|              |
 |                  |<--poi ok--------|              |
 |                  |                 |              |
 |                  | check ContentType = audio/*    |
 |                  |                 |              |
 |                  |--SELECT-------->|  existing PoiLocalization
 |                  |<--existing------|
 |                  |                 |              |
 |                  | [DeleteOldOnReplace flag]       |
 |                  | existing.AudioPublicId != null? |
 |                  | → true          |              |
 |                  |--DeleteAsync(AudioPublicId)---->|
 |                  |                 |              |<--Cloudinary delete
 |                  |                 |              |
 |                  |--UploadAudioAsync(file, sellerId)>|
 |                  |                 |              |--upload to voztrip/audio/{sellerId}
 |                  |                 |              |<--{ Url, PublicId }
 |                  |                 |              |
 |                  |--UPDATE/INSERT->|  PoiLocalization.AudioUrl, AudioPublicId
 |                  |<--ok------------|
 |<--200 { audioUrl }----------------|
```
...
### SD-08 Dịch tự động đa ngôn ngữ

```
Seller             API               DB         LibreTranslate
 |                  |                 |              |
 |--POST /api/seller/pois/{id}/       |              |
 |    localizations/translate-------->|              |
 |  { sourceLanguageId }              |              |
 |                  |                 |              |
 |                  |--SELECT-------->|  Poi + Localizations
 |                  |<--poi + locs----|
 |                  |                 |
 |                  | find source localization
 |                  | (by sourceLanguageId)
 |                  |                 |
 |                  |--SELECT-------->|  Languages WHERE IsActive=true
 |                  |<--all languages-|
 |                  |                 |
 |                  | [SkipExistingManual flag]
 |                  | filter out languages with manual translations
 |                  |                 |
 |                  | TranslateToManyAsync() [parallel]
 |                  |                 |              |
 |                  |----POST /translate (title, vi→en)-->|
 |                  |----POST /translate (title, vi→zh)-->|
 |                  |----POST /translate (desc, vi→en)--->|
 |                  |----POST /translate (desc, vi→zh)--->|
 |                  |                 |              |<--translatedText
 |                  |                 |              |
 |                  | foreach language: UPSERT PoiLocalization
 |                  |   IsAutoTranslated = true
 |                  |--INSERT/UPDATE->|
 |                  |<--ok------------|
 |<--200 { message, translated: N }--|
```

---

### SD-09 Upload media (ảnh / video)
...
Seller             API               DB         Cloudinary
 ...
 |                  | verify POI belongs to seller   |
 |                  | check ContentType              |
 |                  | → "image/*" → Image flag check |
 |                  |                 |              |
 |                  |--UploadImageAsync(file, sellerId)>|
 |                  |                 |              |--quality auto, format auto
 |                  |                 |              |--upload to voztrip/images/{sellerId}
 |                  |                 |              |<--{ Url, PublicId }
 |                  |                 |              |
 |                  |--COUNT PoiMedia>|  sortOrder = count
 |                  |<--count---------|
 |                  |--INSERT-------->|  PoiMedia { Url, PublicId, SortOrder }
 |                  |<--ok------------|
 |<--200 { mediaId, mediaUrl, mediaType }
```

---

### SD-10 Admin xóa media vi phạm

```
Admin              API               DB         Cloudinary
 |                  |                 |              |
 |--DELETE /api/admin/media/{mediaId}>|              |
 |                  |                 |              |
 |                  |--FindAsync----->|  PoiMedia
 |                  |<--media---------|
 |                  |                 |
 |                  | [CleanCloudinary flag]          |
 |                  | media.PublicId != null?          |
 |                  | → true          |              |
 |                  |--DeleteAsync(PublicId, Image/Video)->|
 |                  |                 |              |<--deleted
 |                  |                 |              |
 |                  |--Remove-------->|  PoiMedia
 |                  |--SaveChanges--->|
 |                  |<--ok------------|
 |<--200 { message: "Đã xóa ảnh" }---|
```

---

### SD-11 Admin xóa POI (dọn Cloudinary)
...
Admin              API               DB         Cloudinary
 ...
 |--DELETE /api/admin/pois/{id}------>|              |
 |                  |                 |              |
 |                  |--SELECT-------->|  Poi + Media + Localizations
 |                  |<--poi-----------|
 |                  |                 |
 |                  | [CleanCloudinary flag]          |
 |                  | foreach media:  |              |
 ...
 |                  |   if AudioPublicId != null      |
 |                  |--DeleteAsync(AudioPublicId, Raw)->|
 |                  |                 |              |
 |                  |--Remove Poi---->|  (cascade: Media, Localizations, Questions, VisitLogs)
 |                  |--SaveChanges--->|
 |                  |<--ok------------|
 |<--200 { message: "Đã xóa POI" }---|
```

---

### SD-12 Seller nâng cấp VIP
...
Seller             API               DB
 ...
 |                  |                 |
 |                  | seller.Plan = "vip"
 |                  | seller.PlanUpgradedAt = UtcNow
 |                  |--SaveChanges--->|
 |                  |<--ok------------|
 |<--200 { message: "Nâng cấp VIP thành công!", plan: "vip" }
```
